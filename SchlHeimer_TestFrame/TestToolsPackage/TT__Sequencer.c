...
static bool TT__vStepActive( void )
{
    ...
    {
        if (SHMEM_NUM_OF_CLIENTS == TT__stSequencer.pstCurrCmd->u8Processed)
...
void TT__vSequencerCyclic10ms(void)
{
    ...
    // 20171116 wti: Added SystemEvent option
    uint32 u32SysEvt = SHMEM_u32GetSystemEvent();
    if (u32SysEvt != UINT32_MAX)    // Is there a request for a system event
    {                               // YES
        TT__vSysEvtFunction(u32SysEvt);         // perform system event
        SHMEM_vSetSystemEvent(UINT32_MAX);      // reset request
    }
    // 20171116 wti
