...
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI  
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, 0);
    for (int i = 0; i < 909; i++)               // should be at least 90%
      //HMI::WAS::Framework::PostMessage(HMI::WRS::DEVELOPER_MENU_ACTIVATED, HMI::WCS::enMessageType::GLOBAL_BROADCAST_MESSAGE);
      APIM_vSystemEventOccured_internal(CC_IPC::SI_EV_APIM_SysEv_HUD_BACKLIGHTSTATEACTIVATED);
#endif
}

//-------------------------------------------------------------------------------------------------
static void TT__vFullFillMessageQueue(uint16 u16FuncId)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI  
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, 0);
...
static void TT__vAddMessageQueueDelayItems(uint16 u16FuncId)
{
    ...
    for (int i = 0; i < 55; i++)                // more than 3%
        APIM_vSystemEventOccured_internal(CC_IPC::SI_EV_APIM_SysEv_NAVIGATION_RG_TYPE_CHANGED);
#endif
}
...
static void TT__vAddMessageQueueDiscardItems(uint16 u16FuncId)
{
    ...
    for (int i = 0; i < 55; i++)                // more than 3%
        APIM_vSystemEventOccured_internal(CC_IPC::SI_EV_APIM_SysEv_FREEZE_CANCEL);
...
static void TT__vActivateWarning(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_hmi_kbi_anzeige_id, 0, &u16Param, 4);
#endif
}

static void TT__vVZE_Verkehrszeichen_1(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_hmi_VZE_Verkehrszeichen_1, 0, &u16Param, 4);
#endif
}

static void TT__vVZE_Request(uint16 u16FuncId, bool u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_hmi_VZE_request, 0, &u16Param, 4);
#endif
}

static void TT__vNavigation_available(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_bap_navigation_available, 0, &u16Param, 4);
#endif
}

static void TT__vNavi_CompassInfo_available(uint16 u16FuncId, bool u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_bap_navi_CompassInfo_available, 0, &u16Param, 4);
#endif
}

static void TT__vPresetsStatus(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_hmi_PresetsStatus, 0, &u16Param, 4);
#endif
}

static void TT__vCompassInfo_Direction_Angle(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessHMI) && (IDE_nProcess == SYSCONST_nProcessHMI)      // only for HMI

    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    APIMGR_xTraceCmdSetAPIValue(APIM_nFId_hmi_CompassInfo_Direction_Angle, 0, &u16Param, 4);
#endif
}

//-------------------------------------------------------------------------------------------------
static void TT__vActivateDeveloperMenu(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessLegacy) && (IDE_nProcess == SYSCONST_nProcessLegacy)   // only for Legacy
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    EVHD_vSendEvent(EV_APIM_SysEv_DEVELOPER_MENU_ACTIVATED);

#endif
}

//-------------------------------------------------------------------------------------------------
static void TT__vDeveloperMenu_Down(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessLegacy) && (IDE_nProcess == SYSCONST_nProcessLegacy)   // only for Legacy
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    EVHD_vSendEvent(EV_APIM_SysEv_HK_MENU_RIGHT_SHORT);

#endif
}

//-------------------------------------------------------------------------------------------------
static void TT__vDeveloperMenu_Up(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessLegacy) && (IDE_nProcess == SYSCONST_nProcessLegacy)   // only for Legacy
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    EVHD_vSendEvent(EV_APIM_SysEv_HK_MENU_LEFT_SHORT);

#endif
}

//-------------------------------------------------------------------------------------------------
static void TT__vDeveloperMenu_Press(uint16 u16FuncId, uint16 u16Param)
{
#if (defined SYSCONST_nProcessLegacy) && (IDE_nProcess == SYSCONST_nProcessLegacy)   // only for Legacy
    TT_vTraceEvent(TT_ExecFunction, u16FuncId, u16Param);

    EVHD_vSendEvent(EV_APIM_SysEv_HK_KEY_OK_SHORT_PRESS);
...
void TT__vExecFunction( uint16 u16FuncId, uint16 u16Param )
{
    ...
    case SHMEM_nSeqFunc_FillMessageQueue:
        ...
        break;
    case SHMEM_nSeqFunc_FullFillMessageQueue:
        TT__vFullFillMessageQueue(u16FuncId);
        break;
    ...
    case SHMEM_nSeqFunc_VolumeChange:
        ...
        break;
	case SHMEM_nSeqFunc_ActivateWarning:
        TT__vActivateWarning(u16FuncId, u16Param);
        break;
    case SHMEM_nSeqFunc_VZE_Verkehrszeichen_1:
        TT__vVZE_Verkehrszeichen_1(u16FuncId, u16Param);
        break;
	case SHMEM_nSeqFunc_VZE_Request:
		TT__vVZE_Request(u16FuncId, u16Param);
		break;
    case SHMEM_nSeqFunc_Navigation_available:
        TT__vNavigation_available(u16FuncId, u16Param);
        break;
    case SHMEM_nSeqFunc_Navi_CompassInfo_available:
        TT__vNavi_CompassInfo_available(u16FuncId, u16Param);
        break;
    case SHMEM_nSeqFunc_PresetsStatus:
        TT__vPresetsStatus(u16FuncId, u16Param);
        break;
    case SHMEM_nSeqFunc_CompassInfo_Direction_Angle:
        TT__vCompassInfo_Direction_Angle(u16FuncId, u16Param);
        break;
    case SHMEM_nSeqFunc_ActivateDeveloperMenu:
        TT__vActivateDeveloperMenu(u16FuncId, u16Param);
        break;
	case SHMEM_nSeqFunc_DeveloperMenu_Down:
        TT__vDeveloperMenu_Down(u16FuncId, u16Param);
        break;
	case SHMEM_nSeqFunc_DeveloperMenu_Up:
        TT__vDeveloperMenu_Up(u16FuncId, u16Param);
        break;
	case SHMEM_nSeqFunc_DeveloperMenu_Press:
        TT__vDeveloperMenu_Press(u16FuncId, u16Param);
        break;
    }
}
